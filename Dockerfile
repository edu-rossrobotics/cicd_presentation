...
# copy overlay source
RUN mkdir -p ./src/mk4_ccu
COPY ./ ./src/mk4_ccu

# List the contents of the source path before copying (from the build context)
RUN echo "Contents of the source path:" && ls -la .

# List the contents of the copied path
RUN echo "Contents of the copied path:" && ls -la ./src/mk4_ccu
...
RUN dep_types=(\
    ...
    for dep_type in "${dep_types[@]}"; do \
      ...
      rosdep install -y \
        --from-paths src \
        --ignore-src \
        --skip-keys "$(rosdep keys --from-paths src | grep '^mk4') \
        behaviortree_ros2 \
        file_transfer_action_server" \
        --reinstall \
        --simulate \
        ${2} \
...
# Copy the src directory from the cacher stage
COPY --from=cacher $OVERLAY_WS/src/ $OVERLAY_WS/src/
